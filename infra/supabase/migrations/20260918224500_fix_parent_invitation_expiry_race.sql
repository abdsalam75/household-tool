...
BEGIN
  ...
  IF target_invitation.expires_at <= pg_catalog.clock_timestamp() THEN
  ...
  -- Locks above can block until after an otherwise valid invitation expires.
  -- Capture and re-check time immediately before mutation, never before a
  -- potentially blocking lock acquisition.
